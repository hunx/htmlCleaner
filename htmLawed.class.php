...
class htmLawed {
	...
	*/
	...
	public function setting($key, $value) {
		$this->C[$key] = $value;
		if ($key == 'base_url' && empty($this->C['abs_url'])) {
			$this->C['abs_url'] = 1;
		}
	}
	...
	public function clean() {
		...
		$t = $this->html;
		if (!empty($this->C['valid_xhtml'])) {
			$this->C['elements'] = empty($this->C['elements']) ? '*-center-dir-font-isindex-menu-s-strike-u' : $this->C['elements'];
			$this->C['make_tag_strict'] = isset($this->C['make_tag_strict']) ? $this->C['make_tag_strict'] : 2;
			$this->C['xml:lang'] = isset($this->C['xml:lang']) ? $this->C['xml:lang'] : 2;
		}
		// config eles
		...
		if (!empty($this->C['safe'])) {
			unset($e['applet'], $e['embed'], $e['iframe'], $e['object'], $e['script']);
		}
		$x = !empty($this->C['elements']) ? str_replace(array("\n", "\r", "\t", ' '), '', $this->C['elements']) : '*';
		if ($x == '-*') {
			$e = array();
		...
		$this->C['elements'] =& $e;
		// config attrs
		$x = !empty($this->C['deny_attribute']) ? str_replace(array("\n", "\r", "\t", ' '), '', $this->C['deny_attribute']) : '';
		$x = array_flip((isset($x[0]) && $x[0] == '*') ? explode('-', $x) : explode(',', $x. (!empty($this->C['safe']) ? ',on*' : '')));
		if (isset($x['on*'])) {
			unset($x['on*']);
			$x += array(
				'onblur' =>  1,
		...
		$this->C['deny_attribute'] = $x;

		// config URL
		$x = (isset($this->C['schemes'][2]) && strpos($this->C['schemes'], ':')) ? strtolower($this->C['schemes']) : 'href: aim, feed, file, ftp, gopher, http, https, irc, mailto, news, nntp, sftp, ssh, telnet; *:file, http, https';
		$this->C['schemes'] = array();
		foreach (explode(';', str_replace(array(' ', "\t", "\r", "\n"), '', $x)) as $v) {
			$x = $x2 = null;
			list($x, $x2) = explode(':', $v, 2);
			if ($x2) {
				$this->C['schemes'][$x] = array_flip(explode(',', $x2));
			}
		}
		if (!isset($this->C['schemes']['*'])) {
			$this->C['schemes']['*'] = array('file' => 1, 'http' => 1, 'https' => 1,);
		}
		if (!empty($this->C['safe']) && empty($this->C['schemes']['style'])) {
			$this->C['schemes']['style'] = array('!' => 1);
		}
		$this->C['abs_url'] = isset($this->C['abs_url']) ? $this->C['abs_url'] : 0;
		if (!isset($this->C['base_url']) || !preg_match('`^[a-zA-Z\d.+\-]+://[^/]+/(.+?/)?$`', $this->C['base_url'])) {
			$this->C['base_url'] = $this->C['abs_url'] = 0;
		}
		// config rest
		$this->C['and_mark'] = empty($this->C['and_mark']) ? 0 : 1;
		$this->C['anti_link_spam'] = (isset($this->C['anti_link_spam']) && is_array($this->C['anti_link_spam']) && count($this->C['anti_link_spam']) == 2 && (empty($this->C['anti_link_spam'][0]) || $this->hl_regex($this->C['anti_link_spam'][0])) && (empty($this->C['anti_link_spam'][1]) || $this->hl_regex($this->C['anti_link_spam'][1]))) ? $this->C['anti_link_spam'] : 0;
		$this->C['anti_mail_spam'] = isset($this->C['anti_mail_spam']) ? $this->C['anti_mail_spam'] : 0;
		$this->C['balance'] = isset($this->C['balance']) ? (bool)$this->C['balance'] : 1;
		$this->C['cdata'] = isset($this->C['cdata']) ? $this->C['cdata'] : (empty($this->C['safe']) ? 3 : 0);
		$this->C['clean_ms_char'] = empty($this->C['clean_ms_char']) ? 0 : $this->C['clean_ms_char'];
		$this->C['comment'] = isset($this->C['comment']) ? $this->C['comment'] : (empty($this->C['safe']) ? 3 : 0);
		$this->C['css_expression'] = empty($this->C['css_expression']) ? 0 : 1;
		$this->C['direct_list_nest'] = empty($this->C['direct_list_nest']) ? 0 : 1;
		$this->C['hexdec_entity'] = isset($this->C['hexdec_entity']) ? $this->C['hexdec_entity'] : 1;
		$this->C['hook'] = (!empty($this->C['hook']) && function_exists($this->C['hook'])) ? $this->C['hook'] : 0;
		$this->C['hook_tag'] = (!empty($this->C['hook_tag']) && function_exists($this->C['hook_tag'])) ? $this->C['hook_tag'] : 0;
		$this->C['keep_bad'] = isset($this->C['keep_bad']) ? $this->C['keep_bad'] : 6;
		$this->C['lc_std_val'] = isset($this->C['lc_std_val']) ? (bool)$this->C['lc_std_val'] : 1;
		$this->C['make_tag_strict'] = isset($this->C['make_tag_strict']) ? $this->C['make_tag_strict'] : 1;
		$this->C['named_entity'] = isset($this->C['named_entity']) ? (bool)$this->C['named_entity'] : 1;
		$this->C['no_deprecated_attr'] = isset($this->C['no_deprecated_attr']) ? $this->C['no_deprecated_attr'] : 1;
		$this->C['parent'] = isset($this->C['parent'][0]) ? strtolower($this->C['parent']) : 'body';
		$this->C['show_setting'] = !empty($this->C['show_setting']) ? $this->C['show_setting'] : 0;
		$this->C['style_pass'] = empty($this->C['style_pass']) ? 0 : 1;
		$this->C['tidy'] = empty($this->C['tidy']) ? 0 : $this->C['tidy'];
		$this->C['unique_ids'] = isset($this->C['unique_ids']) ? $this->C['unique_ids'] : 1;
		$this->C['xml:lang'] = isset($this->C['xml:lang']) ? $this->C['xml:lang'] : 0;

		$S = is_array($this->S) ? $this->S : $this->hl_spec($this->S);

		$t = preg_replace('`[\x00-\x08\x0b-\x0c\x0e-\x1f]`', '', $t);
		if ($this->C['clean_ms_char']) {
			...
			$x = $x + ($this->C['clean_ms_char'] == 1 ? array("\x82" => '&#8218;', "\x84" => '&#8222;', "\x91" => '&#8216;', "\x92" => '&#8217;', "\x93" => '&#8220;', "\x94" => '&#8221;') : array("\x82" => '\'', "\x84" => '"', "\x91" => '\'', "\x92" => '\'', "\x93" => '"', "\x94" => '"'));
			$t = strtr($t, $x);
		}
		if ($this->C['cdata'] || $this->C['comment']) {
			$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', array(get_class($this), 'hl_cmtcd'), $t);
		}
		$t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', array(get_class($this), 'hl_ent'), str_replace('&', '&amp;', $t));
		if ($this->C['unique_ids'] && !isset($this->hl_Ids)) {
			$this->hl_Ids = array();
		}
		if ($this->C['hook']) {
			$t = $this->C['hook']($t, $this->C, $S);
		}
		if ($this->C['show_setting'] && preg_match('`^[a-z][a-z0-9_]*$`i', $this->C['show_setting'])) {
			$this->C['show_setting'] = array(
				'config' => $this->C,
				'spec' => $S,
				'time' => microtime(),
			);
		...
		// main
		$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', array(get_class($this), 'hl_tag'), $t);
		$t = $this->C['balance'] ? $this->hl_bal($t, $this->C['keep_bad'], $this->C['parent']) : $t;
		$t = (($this->C['cdata'] || $this->C['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
		$t = $this->C['tidy'] ? $this->hl_tidy($t, $this->C['tidy'], $this->C['parent']) : $t;
		return $t;
		// eof
	}
	 ...
	 *
	 */
	private function hl_attrval($t, $p) {
	...
	*
	...
	private function hl_bal($t, $do=1, $in='div') {
		...
		$cE = array('area' => 1, 'br' => 1, 'col' => 1, 'embed' => 1, 'hr' => 1, 'img' => 1, 'input' => 1, 'isindex' => 1, 'param' => 1);
		// Empty
		$cF = array('button' => 1, 'del' => 1, 'div' => 1, 'dd' => 1, 'fieldset' => 1, 'iframe' => 1, 'ins' => 1, 'li' => 1, 'noscript' => 1, 'object' => 1, 'td' => 1, 'th' => 1);
		// Flow; later context-wise dynamic move of ins & del to $cI
		$cI = array('a' => 1, 'abbr' => 1, 'acronym' => 1, 'address' => 1, 'b' => 1, 'bdo' => 1, 'big' => 1, 'caption' => 1, 'cite' => 1, 'code' => 1, 'dfn' => 1, 'dt' => 1, 'em' => 1, 'font' => 1, 'h1' => 1, 'h2' => 1, 'h3' => 1, 'h4' => 1, 'h5' => 1, 'h6' => 1, 'i' => 1, 'kbd' => 1, 'label' => 1, 'legend' => 1, 'p' => 1, 'pre' => 1, 'q' => 1, 'rb' => 1, 'rt' => 1, 's' => 1, 'samp' => 1, 'small' => 1, 'span' => 1, 'strike' => 1, 'strong' => 1, 'sub' => 1, 'sup' => 1, 'tt' => 1, 'u' => 1, 'var' => 1);
		// Inline
		...
		if ($this->C['direct_list_nest']) {
			$cS['ol'] = $cS['ul'] += array('ol' => 
			1, 'ul' => 
			1);
		}
		...
		$cT = array('colgroup' => 1, 'dd' => 1, 'dt' => 1, 'li' => 1, 'option' => 1, 'p' => 1, 'td' => 1, 'tfoot' => 1, 'th' => 1, 'thead' => 1, 'tr' => 1);
		// Omitable closing
		...
		$eO = array('area' => 1, 'caption' => 1, 'col' => 1, 'colgroup' => 1, 'dd' => 1, 'dt' => 1, 'legend' => 1, 'li' => 1, 'optgroup' => 1, 'option' => 1, 'param' => 1, 'rb' => 1, 'rbc' => 1, 'rp' => 1, 'rt' => 1, 'rtc' => 1, 'script' => 1, 'tbody' => 1, 'td' => 1, 'tfoot' => 1, 'thead' => 1, 'th' => 1, 'tr' => 1);
		// Missing in $eB & $eI
		$eF = $eB + $eI;
		// $in sets allowed child
		$in = ((isset($eF[$in]) && $in != '#pcdata') || isset($eO[$in])) ? $in : 'div';
		if (isset($cE[$in])) {
		...
		for ($i=-1, $ci=count($t); ++$i<$ci;) {
			...
			if (isset($e) && ($do == 1 || (isset($ok['#pcdata']) && ($do == 3 || $do == 5)))) {
				$output .= '<' . $s . $e . $a . '>';
			}
			if (isset($x[0])) {
				if ($do < 3 || isset($ok['#pcdata'])) {
					$output .= $x;
				} elseif (strpos($x, "\x02\x04")) {
					foreach (preg_split('`(\x01\x02[^\x01\x02]+\x02\x01)`', $x, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $v) {
						$output .= (substr($v, 0, 2) == "\x01\x02" ? $v : ($do > 4 ? preg_replace('`\S`', '', $v) : ''));
					}
				} elseif ($do > 4) {
	 ...
	 *
	 */
	private function hl_cmtcd($t) {
		// comment/CDATA sec handler
		$t = $t[0];
		if (!($v = $this->C[$n = $t[3] == '-' ? 'comment' : 'cdata'])) {
			return $t;
		}
		if ($v == 1) {
			return '';
	...
	private function hl_ent($t) {
		...
		if ($t[0] != '#') {
			return ($this->C['and_mark'] ? "\x06" : '&'). (isset($U[$t]) ? $t : (isset($N[$t]) ? (!$this->C['named_entity'] ? '#'. ($this->C['hexdec_entity'] > 1 ? 'x'. dechex($N[$t]) : $N[$t]) : $t) : 'amp;'. $t)). ';';
		}
		if (($n = ctype_digit($t = substr($t, 1)) ? intval($t) : hexdec(substr($t, 1))) < 9 || ($n > 13 && $n < 32) || $n == 11 || $n == 12 || ($n > 126 && $n < 160 && $n != 133) || ($n > 55295 && ($n < 57344 || ($n > 64975 && $n < 64992) || $n == 65534 || $n == 65535 || $n > 1114111))) {
			return ($this->C['and_mark'] ? "\x06" : '&'). "amp;#{$t};";
		}
		return ($this->C['and_mark'] ? "\x06" : '&'). '#'. (((ctype_digit($t) && $this->C['hexdec_entity'] < 2) || !$this->C['hexdec_entity']) ? $n : 'x'. dechex($n)). ';';
		// eof
	}
	 ...
	 *
	 */
	private function hl_prot($p, $c=null) {
		// check URL scheme
		$b = $a = '';
		if ($c == null) {
			$c = 'style';
		...
		$c = isset($this->C['schemes'][$c]) ? $this->C['schemes'][$c] : $this->C['schemes']['*'];
		static $d = 'denied:';
		...
		if ($this->C['abs_url']) {
			if ($this->C['abs_url'] == -1 && strpos($p, $this->C['base_url']) === 0) {
				// Make url rel
				$p = substr($p, strlen($this->C['base_url']));
			} elseif (empty($m[1])) {
				// Make URL abs
				if (substr($p, 0, 2) == '//') {
					$p = substr($this->C['base_url'], 0, strpos($this->C['base_url'], ':')+1). $p;
				} elseif ($p[0] == '/') {
					$p = preg_replace('`(^.+?://[^/]+)(.*)`', '$1', $this->C['base_url']). $p;
				} elseif (strcspn($p, './')) {
					$p = $this->C['base_url']. $p;
				} else {
					preg_match('`^([a-zA-Z\d\-+.]+://[^/]+)(.*)`', $this->C['base_url'], $m);
	...
	private function hl_tag($t) {
		...
		} elseif (!isset($this->C['elements'][($e = strtolower($m[2]))])) {
			return (($this->C['keep_bad']%2) ? str_replace(array('<', '>'), array('<', '>'), $t) : '');
		}
		// attr string
		$a = str_replace(array("\n", "\r", "\t"), ' ', trim($m[3]));
		// tag transform
		static $eD = array('applet' => 1, 'center' => 1, 'dir' => 1, 'embed' => 1, 'font' => 1, 'isindex' => 1, 'menu' => 1, 's' => 1, 'strike' => 1, 'u' => 1);
		// Deprecated
		if ($this->C['make_tag_strict'] && isset($eD[$e])) {
			$trt = $this->hl_tag2($e, $a, $this->C['make_tag_strict']);
			if (!$e) {
				return (($this->C['keep_bad']%2) ? str_replace(array('<', '>'), array('<', '>'), $t) : '');
			}
		}
		...
		// Empty ele
		if (!empty($m[1])) {
			return (!isset($eE[$e]) ? (empty($this->C['hook_tag']) ? "</$e>" : $this->C['hook_tag']($e)) : (($this->C['keep_bad'])%2 ? str_replace(array('<', '>'), array('<', '>'), $t) : ''));
		}
		// open tag & attr
		...
		static $aNE = array('checked' => 1, 'compact' => 1, 'declare' => 1, 'defer' => 1, 'disabled' => 1, 'ismap' => 1, 'multiple' => 1, 'nohref' => 1, 'noresize' => 1, 'noshade' => 1, 'nowrap' => 1, 'readonly' => 1, 'selected' => 1);
		// Empty
		static $aNP = array('action' => 1, 'cite' => 1, 'classid' => 1, 'codebase' => 1, 'data' => 1, 'href' => 1, 'longdesc' => 1, 'model' => 1, 'pluginspage' => 1, 'pluginurl' => 1, 'usemap' => 1);
		// Need scheme check; excludes style, on* & src
		static $aNU = array(
			'class' => array('param' => 1, 'script' => 1),
			'dir' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'iframe' => 1, 'param' => 1, 'script' => 1),
			'id' => array('script' => 1),
			'lang' => array('applet' => 1, 'br' => 1, 'iframe' => 1, 'param' => 1, 'script' => 1),
			'xml:lang' => array('applet' => 1, 'br' => 1, 'iframe' => 1, 'param' => 1, 'script' => 1),
			'onclick' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'ondblclick' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onkeydown' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onkeypress' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onkeyup' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onmousedown' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onmousemove' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onmouseout' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onmouseover' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'onmouseup' => array('applet' => 1, 'bdo' => 1, 'br' => 1, 'font' => 1, 'iframe' => 1, 'isindex' => 1, 'param' => 1, 'script' => 1),
			'style' => array('param' => 1, 'script' => 1),
			'title' => array('param' => 1, 'script' => 1),
		);
		// Univ & exceptions
		if ($this->C['lc_std_val']) {
		...
		$nfr = 0;
		foreach ($aA as $k => $v) {
			if (((isset($this->C['deny_attribute']['*']) ? isset($this->C['deny_attribute'][$k]) : !isset($this->C['deny_attribute'][$k])) || isset($rl[$k])) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) || isset($rl[$k])) && (isset($aN[$k][$e]) || (isset($aNU[$k]) && !isset($aNU[$k][$e])))) {
				if (isset($aNE[$k])) {
					$v = $k;
				} elseif (!empty($lcase) && (($e != 'button' || $e != 'input') || $k == 'type')) {
					// Rather loose but ?not cause issues
					$v = (isset($aNL[($v2 = strtolower($v))])) ? $v2 : $v;
				}
				if ($k == 'style' && !$this->C['style_pass']) {
					if (false !== strpos($v, '&#')) {
					...
					$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', array(get_class($this), 'hl_prot'), $v);
					$v = !$this->C['css_expression'] ? preg_replace('`expression`i', ' ', preg_replace('`\\\\\S|(/|(%2f))(\*|(%2a))`i', ' ', $v)) : $v;
				} elseif (isset($aNP[$k]) || strpos($k, 'src') !== false || $k[0] == 'o') {
					$v = str_replace("\xad", ' ', (strpos($v, '&') !== false ? str_replace(array('&#xad;', '&#173;', '&shy;'), ' ', $v) : $v));
					$v = $this->hl_prot($v, $k);
					if ($k == 'href') {
						// X-spam
						if ($this->C['anti_mail_spam'] && strpos($v, 'mailto:') === 0) {
							$v = str_replace('@', htmlspecialchars($this->C['anti_mail_spam']), $v);
						} elseif ($this->C['anti_link_spam']) {
							$r1 = $this->C['anti_link_spam'][1];
							if (!empty($r1) && preg_match($r1, $v)) {
								continue;
							}
							$r0 = $this->C['anti_link_spam'][0];
				...
				if (isset($rl[$k]) && is_array($rl[$k]) && ($v = $this->hl_attrval($v, $rl[$k])) === 0) {
					continue;
				}
				$a[$k] = str_replace('"', '&quot;', $v);
			}
		...
		if ($depTr) {
			...
			foreach ($a as $k => $v) {
				...
				if ($k == 'align') {
					...
					if ($this->C['no_deprecated_attr'] == 2 || ($e != 'a' && $e != 'map')) {
						unset($a['name']);
					}
		...
		// unique ID
		if ($this->C['unique_ids'] && isset($a['id'])) {
			if (!preg_match('`^[A-Za-z][A-Za-z0-9_\-.:]*$`', ($id = $a['id'])) || (isset($this->hl_Ids[$id]) && $this->C['unique_ids'] == 1)) {
				unset($a['id']);
			} else {
				while (isset($this->hl_Ids[$id])) {
					$id = $this->C['unique_ids']. $id;
				}
				$this->hl_Ids[($a['id'] = $id)] = 1;
			}
		}
		// xml:lang
		if ($this->C['xml:lang'] && isset($a['lang'])) {
			$a['xml:lang'] = isset($a['xml:lang']) ? $a['xml:lang'] : $a['lang'];
			if ($this->C['xml:lang'] == 2) {
				unset($a['lang']);
			}
		}
		// for transformed tag
	 ...
	 *
	 */
	private function hl_tag2(&$e, &$a, $t=1) {
		// transform tag
		if ($e == 'center') {
			$e = 'div';
	...
	private function hl_tidy($t, $w, $p) {
		...
		$output = '';
		if (isset($d[$p])) {
			$output .= str_repeat($s, ++$n);
		}
		$t = explode('<', $t);
		$output .= ltrim(array_shift($t));
